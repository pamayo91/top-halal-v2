...
class ContactController extends Controller
{
    public function create(Request $request): View
    {
        ...
        abort_unless($reference === null || preg_match('/^TH-PROP-[A-Z0-9]{12}$/', $reference) === 1, 404);

        return view('public.contact.create', [
            'prefillSubject' => $reference ? 'Contestation d’un refus de proposition' : null,
    ...
    public function store(StoreContactMessageRequest $request, TransactionalMailService $mail): RedirectResponse
    {
        ...
        $data=$request->validated(); $context = filled($data['submission_reference'] ?? null) ? "Référence proposition : {$data['submission_reference']}\n\n" : ''; $contact=ContactMessage::create(['name'=>trim($data['name']),'email'=>strtolower(trim($data['email'])),'subject'=>trim($data['subject']),'message'=>$context.trim($data['message']),'ip_hash'=>hash_hmac('sha256',(string)$request->ip(),(string)config('app.key'))]);

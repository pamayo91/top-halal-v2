...
class LegacyMigrateAllCommand extends Command
{
    ...
    private function migrateMedia(string $phase): void
    {
        ...
        foreach($inline['items'] as $item)if($item['outcome']!=='ready')$this->anomaly($phase,$item['legacy_wp_id'].':'.$item['position'],(string)$item['outcome'],$item['outcome']==='processing_error'?'error':'warning',$item);

...
use App\Services\{ContentSanitizer,ContentTransformer,LegacyContentReader,LegacyMediaReader,LegacyRestaurantMigrator,LegacyReviewMigrator,LegacyReviewReader,MediaVariantGenerator};
...
class LegacyMigrateAllCommand extends Command
{
    ...
    private function migrateContent(string $phase,string $type,string $model): void
    {
        ...
        $this->idBatches($phase,'posts',fn($q)=>$q->where('post_type',$type)->whereIn('post_status',['publish','pending','draft']),function($id)use($reader,$transformer,$sanitizer,$type,$model){$x=$reader->read($type,$id);$post=$x['post'];$t=$transformer->transform($post->post_content);$s=$sanitizer->sanitize($t['html']);$slug=Str::slug($post->post_name?:$post->post_title)?:"$type-$id";$anomalies=array_merge($t['unknown'],$s['removed']);if($this->apply){if($model::where('slug',$slug)->where('legacy_wp_id','!=',$id)->exists()){$slug.="-$id";$anomalies[]='duplicate_slug_resolved';}$exists=$model::where('legacy_wp_id',$id)->exists();$o=$model::updateOrCreate(['legacy_wp_id'=>$id],['legacy_author_id'=>$post->post_author?:null,'original_title'=>$post->post_title,'title'=>$post->post_title?:'Sans titre','slug'=>$slug,'content_html'=>$s['html'],'status'=>$post->post_status==='publish'?'published':$post->post_status,'legacy_url'=>'/'.ltrim($post->post_name,'/').'/', 'legacy_published_at'=>$post->post_date_gmt,'legacy_modified_at'=>$post->post_modified_gmt]);if($type==='post'){$cats=[];$tags=[];foreach($x['terms'] as $term){$class=$term->taxonomy==='category'?EditorialCategory::class:EditorialTag::class;$id2=$class::updateOrCreate(['legacy_term_id'=>$term->term_id],['name'=>$term->name,'slug'=>Str::slug($term->slug?:$term->name)?:"term-$term->term_id"])->id;($term->taxonomy==='category'?$cats:$tags)[]=$id2;}$o->categories()->sync($cats);$o->tags()->sync($tags);}if($featured=$x['meta']['_thumbnail_id']??null)ContentMedia::updateOrCreate(['content_type'=>$type,'content_id'=>$o->id,'legacy_attachment_id'=>(int)$featured,'role'=>'featured'],[]);return [$exists?'existing':'migrated',$anomalies];}return ['migrated',$anomalies];});
    }

    private function migrateComments(string $phase): void
    {
        $this->idBatches($phase,'comments',fn($q)=>$q->whereIn('comment_approved',['0','1'])->whereIn('comment_type',['','comment']),function($id){$row=DB::connection('legacy_wp')->table('comments')->where('comment_ID',$id)->first();$article=Article::where('legacy_wp_id',$row->comment_post_ID)->first();$page=$article?null:Page::where('legacy_wp_id',$row->comment_post_ID)->first();if(!$article&&!$page)return ['ignored',['unmigrated_or_unsupported_post']];if($this->apply){$exists=Comment::where('legacy_wp_comment_id',$id)->exists();Comment::updateOrCreate(['legacy_wp_comment_id'=>$id],['legacy_wp_post_id'=>$row->comment_post_ID,'article_id'=>$article?->id,'page_id'=>$page?->id,'legacy_user_id'=>$row->user_id?:null,'author_name'=>trim(strip_tags($row->comment_author))?:'Anonyme','author_email'=>$row->comment_author_email?:null,'content'=>trim(html_entity_decode(strip_tags($row->comment_content),ENT_QUOTES|ENT_HTML5,'UTF-8')),'status'=>$row->comment_approved==='1'?'approved':'pending','approved_at'=>$row->comment_approved==='1'?$row->comment_date_gmt:null,'created_at'=>$row->comment_date_gmt,'updated_at'=>$row->comment_date_gmt]);return [$exists?'existing':'migrated',[]];}return ['migrated',[]];});
    ...
    private function migrateReviews(string $phase): void
    {
        ...
        $this->idBatches($phase,'posts',fn($q)=>$q->where('post_type','lp-reviews'),function($id)use($reader,$migrator){$post=$reader->findMany([$id])[0]??null;if(!$post)return ['errors',['review_not_found']];$record=$migrator->inspect($post);if($record['anomalies']!==[])return ['ignored',$record['anomalies']];if($this->apply){$exists=RestaurantReview::where('legacy_wp_review_id',$id)->exists();$migrator->persist($record);return[$exists?'existing':'migrated',[]];}return['migrated',[]];});
    }

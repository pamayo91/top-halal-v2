...
class LegacyMigrateAllCommand extends Command
{
    ...
    private function migrateTaxonomies(string $phase): void
    {
        ...
        foreach (['listing-category'=>[\App\Models\Category::class,'categories'],'features'=>[\App\Models\Feature::class,'features']] as $taxonomy=>[$model,$table]) {
            $db->table('term_taxonomy as x')->join('terms as t','t.term_id','=','x.term_id')->where('x.taxonomy',$taxonomy)->orderBy('t.term_id')->select('t.term_id','t.name','t.slug','x.parent')->chunkById($this->batch,function($rows)use($model,$taxonomy,&$counts,$phase){foreach($rows as $term){$counts['source']++;$counts['examined']++;if(app(\App\Services\TaxonomyValueClassifier::class)->isMalicious($term->name)){$counts['ignored']++;$counts['anomalies']++;$this->anomaly($phase,$term->term_id,'malicious_taxonomy_term_excluded','warning',['taxonomy'=>$taxonomy]);continue;}if(!$this->apply){$counts['migrated']++;continue;} $exists=$model::where('legacy_term_id',$term->term_id)->exists();$data=['name'=>trim($term->name)?:'Sans nom','slug'=>Str::slug($term->slug?:$term->name)?:"term-$term->term_id"];$model::updateOrCreate(['legacy_term_id'=>$term->term_id],$data);$counts[$exists?'existing':'migrated']++;}} ,'t.term_id','term_id');
        }
    ...
    private function legacyRole(int $legacyUserId): string { return DB::connection('legacy_wp')->table('usermeta')->where('user_id',$legacyUserId)->where('meta_key','like','%capabilities')->where('meta_value','like','%administrator%')->exists() ? 'admin' : 'user'; }

...
class AuditQuickRestaurantsCommand extends Command
{
    ...
    private function markdown(array $rows): string
    {
        ...
        $out[]=''; $out=array_merge($out,$this->markdownTable('Quick absents de Top-Halal',['Nom','CP','Ville','Halal','Source'],collect($rows)->where('match_status','MISSING_TOP_HALAL')->map(fn($r)=>[$r['quick_name'],$r['quick_postal_code'],$r['quick_city'],$r['quick_halal_status'],$r['quick_url']])->all()));
        $out=array_merge($out,$this->markdownTable('Quick nécessitant une mise à jour',['ID Top-Halal','Quick','Champ','Valeur Top-Halal','Valeur Quick'],collect($rows)->where('match_status','MATCH_UPDATE')->flatMap(function($r){return collect(explode(',',$r['differences']))->filter()->map(fn($field)=>[$r['top_halal_restaurant_id'],$r['quick_name'],$field,$this->topValue($r,$field),$this->quickValue($r,$field)]);})->all()));
        $out=array_merge($out,$this->markdownTable('Correspondances incertaines',['Quick','Restaurant Top-Halal candidat','Motif du doute'],collect($rows)->where('match_status','MATCH_PROBABLE')->map(fn($r)=>[$r['quick_name'],$r['top_halal_name'],'score '.$r['match_score'].'; '.($r['differences'] ?: 'signaux insuffisants')])->all()));
        $out=array_merge($out,$this->markdownTable('Doublons potentiels',['Quick','IDs Top-Halal concernés'],collect($rows)->where('match_status','DUPLICATE_TOP_HALAL')->map(fn($r)=>[$r['quick_name'],implode(', ',$r['_candidate_ids'])])->all()));
        return implode("\n",$out)."\n";
    }
    ...
    private function markdownTable(string $title,array $headers,array $rows): array { $out=['## '.$title,'','| '.implode(' | ',$headers).' |','| '.implode(' | ',array_fill(0,count($headers),'---')).' |']; foreach($rows as $row)$out[]='| '.implode(' | ',array_map(fn($v)=>str_replace('|','\\|',(string)$v),$row)).' |'; return [...$out,'']; }
}

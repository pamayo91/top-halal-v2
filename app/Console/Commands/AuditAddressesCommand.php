...
class AuditAddressesCommand extends Command
{
    ...
    private function summarize($rows): array
    {
        ...
        foreach ($rows as $r) {
            ...
            $locationMismatch = $city && $r->locations !== [] && ! collect($r->locations)->contains(fn ($l) => $this->normal($l['name']) === $this->normal($r->derived_city_name));
    ...
    private function markdown(string $run, array $s, array $legacy, array $sample, int $limit): string
    {
        ...
        $lines = array_merge($lines, ['## Index actuels et recommandations', '', '- Actuels : index sur `status`, `postal_code`, `city_name`, unicité `legacy_wp_id`; aucune indexation GPS dédiée. Les pivots ont une clé primaire `(restaurant_id, location_id)`.', '- La recherche de proximité doit utiliser uniquement des coordonnées non nulles dans les plages valides.', '', '## Modèle de localisation', '', '- Conserver `address` comme adresse historique pendant la transition et utiliser `address_line1`, `address_line2`, `postal_code`, `city_name`, `city_code`, `country_code`, `latitude`, `longitude` pour la localisation structurée.', '- Les régions, départements et codes INSEE doivent provenir d’une source de référence versionnée et ne doivent pas être déduits aveuglément du texte.', '', '## Échantillon déterministe', '', "Le CSV associé contient ".count($sample)." enregistrement(s), plafonné à {$limit}; sélection par classification, anomalies puis `legacy_wp_id` croissant."]);
        return implode("\n", $lines)."\n";
    }

    private function writeCsv(string $path, array $sample): void { $out = fopen($path, 'w'); fputcsv($out, ['v2_id','legacy_wp_id','status','name','address','stored_postal_code','stored_city_name','derived_postal_code','derived_city_name','latitude','longitude','geography','classification','issues','legacy_published_at']); foreach ($sample as $r) fputcsv($out, [$r->id,$r->legacy_wp_id,$r->status,$r->name,$r->address,$r->postal_code,$r->city_name,$r->derived_postal_code,$r->derived_city_name,$r->latitude,$r->longitude,implode(' | ', array_column($r->locations, 'name')),$r->classification,implode(' | ', $r->issues),$r->legacy_published_at]); fclose($out); }

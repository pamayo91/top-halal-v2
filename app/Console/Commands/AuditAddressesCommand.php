...
class AuditAddressesCommand extends Command
{
    ...
    protected $description = 'Read-only audit of migrated restaurant address and GPS data.';
    ...
    public function handle(): int
    {
        ...
        $rows = DB::table('restaurants')
            ->select('id', 'legacy_wp_id', 'name', 'status', 'address', 'postal_code', 'city_name', 'latitude', 'longitude', 'legacy_published_at')
            ->orderBy('id')->get();
    ...
    {
        $s = array_fill_keys(['total','published','other_status','address_present','address_empty','city_present','postal_present','stored_city_present','stored_postal_present','complete_address','address_no_postal','address_no_city','city_only','postal_only','gps_present','latitude_only','longitude_only','gps_absent','gps_zero','gps_out_of_range','gps_swapped','gps_france','gps_dom','gps_outside_france','invalid_text','conflict','invalid','duplicate_coordinate_restaurants'], 0);
        ...
        foreach ($rows as $r) {
            ...
            $derived = $this->extractAddressParts((string) $r->address); $r->derived_postal_code = $r->postal_code ?: $derived['postal_code']; $r->derived_city_name = $r->city_name ?: $derived['city_name'];
            $address = $this->present($r->address); $city = $this->present($r->derived_city_name); $postal = $this->present($r->derived_postal_code);
            $s[$address ? 'address_present' : 'address_empty']++; if ($city) $s['city_present']++; if ($postal) $s['postal_present']++;
            if ($this->present($r->city_name)) $s['stored_city_present']++; if ($this->present($r->postal_code)) $s['stored_postal_present']++;
            ...
            $class = $gpsIssue ? 'CONFLICT' : ($address && $city && $postal ? ($gps ? 'COMPLETE_WITH_GPS' : 'COMPLETE_NO_GPS') : (!$address && !$city && !$postal && $gps ? 'GPS_ONLY' : (!$address && $city && !$postal ? 'CITY_ONLY' : (($address || $city || $postal) ? ($gps ? 'PARTIAL_WITH_GPS' : 'PARTIAL_NO_GPS') : 'INVALID'))));
            if ($class === 'CONFLICT') $s['conflict']++; if ($class === 'INVALID') $s['invalid']++;
            $r->classification = $class; $r->issues = array_merge($textIssues, $gpsIssue ? ['gps_suspect'] : []);
    ...
    {
        $g = fn ($k) => $s[$k] ?? 0;
        $classes = array_merge(array_fill_keys(['COMPLETE_WITH_GPS','COMPLETE_NO_GPS','PARTIAL_WITH_GPS','PARTIAL_NO_GPS','CITY_ONLY','GPS_ONLY','INVALID'], 0), $s['classes']);
        $legacyPostmeta = $this->legacyTable('postmeta');
        $lines = ['# Audit adresses et GPS — Top-Halal V2', '', "Run UUID : `{$run}`", 'Mode : `strictement lecture seule` (les seules écritures sont les deux artefacts de rapport).', '', '## Résumé', '', "TOTAL RESTAURANTS : {$g('total')}", '', "- Adresse complète + GPS : {$classes['COMPLETE_WITH_GPS']}", "- Adresse complète sans GPS : {$classes['COMPLETE_NO_GPS']}", "- Adresse partielle + GPS : {$classes['PARTIAL_WITH_GPS']}", "- Adresse partielle sans GPS : {$classes['PARTIAL_NO_GPS']}", "- Ville seule : {$classes['CITY_ONLY']}", "- GPS seul : {$classes['GPS_ONLY']}", "- Sans adresse ni GPS / invalide : {$classes['INVALID']}", "- Conflits locaux : {$g('conflict')}", '', "- Coordonnées présentes : {$g('gps_present')}", "- Coordonnées absentes : {$g('gps_absent')}", "- Coordonnées invalides ou suspectes : ".($g('gps_zero') + $g('gps_out_of_range') + $g('gps_swapped')), "- Restaurants à coordonnées dupliquées : {$g('duplicate_coordinate_restaurants')}", '', '## Inventaire des champs', '', '### V2', '', '| Table | Champ | Type / nullable | Exemple |', '| --- | --- | --- | --- |', '| restaurants | address | varchar / nullable | valeur non exposée dans ce rapport |', '| restaurants | postal_code | varchar(20) / nullable, indexé | valeur non exposée |', '| restaurants | city_name | varchar / nullable, indexé | valeur non exposée |', '| restaurants | latitude, longitude | decimal(10,7) / nullable | valeur non exposée |', '| restaurants | legacy_wp_id | bigint / non nullable, unique | identifiant de réconciliation |', '', '### Legacy WordPress', '', "- Listings lus : {$legacy['listing_count']}. Les coordonnées/adresses sont stockées dans `{$legacyPostmeta}` (notamment les payloads sérialisés `lp_listingpro_options` / `lp_listingpro_options_fields`), pas dans les options WordPress globales.", '- Champs physiques pertinents : `posts.ID`, `posts.post_status`, `posts.post_name`, `postmeta.post_id/meta_key/meta_value`, `terms.name/slug`, `term_taxonomy.taxonomy/parent`.', '- Aucun champ INSEE, département, région, pays, ligne 2 ou précision de géocodage n’existe dans le modèle V2 actuel.', '', 'Meta keys legacy candidates (occurrences) :'];
        foreach ($legacy['meta_key_candidates'] as $x) $lines[] = "- `{$x['meta_key']}` : {$x['occurrences']}";
        $lines = array_merge($lines, ['', '## Compteurs détaillés', '', "- Publiés : {$g('published')} ; autres statuts : {$g('other_status')}.", "- Adresse présente : {$g('address_present')} ; vide : {$g('address_empty')}. Champs V2 dédiés renseignés : ville {$g('stored_city_present')} ; code postal {$g('stored_postal_present')}.", "- Après extraction read-only du suffixe `CP ville` de l’adresse brute : ville {$g('city_present')} ; code postal {$g('postal_present')} ; adresse + CP + ville : {$g('complete_address')} ; adresse sans CP : {$g('address_no_postal')} ; adresse sans ville : {$g('address_no_city')} ; CP seul : {$g('postal_only')}.", "- Latitude seule : {$g('latitude_only')} ; longitude seule : {$g('longitude_only')} ; 0,0 : {$g('gps_zero')} ; hors plage GPS : {$g('gps_out_of_range')} ; possiblement inversées : {$g('gps_swapped')}.", "- France métropolitaine : {$g('gps_france')} ; DOM/TOM : {$g('gps_dom')} ; hors France : {$g('gps_outside_france')}.", "- Coordonnées uniques : {$g('coordinate_unique')} ; utilisées par 2 : {$g('coordinate_used_by_2')} ; par 3+ : {$g('coordinate_used_by_3_plus')}.", '', '## Anomalies (exemples plafonnés à 20)', '']);
        ...
        $lines = array_merge($lines, ['## Index actuels et recommandations', '', '- Actuels : index sur `status`, `postal_code`, `city_name`, unicité `legacy_wp_id`; aucune indexation GPS dédiée.', '- La recherche de proximité doit utiliser uniquement des coordonnées non nulles dans les plages valides.', '', '## Modèle de localisation', '', '- Conserver `address` comme adresse historique pendant la transition et utiliser `address_line1`, `address_line2`, `postal_code`, `city_name`, `city_code`, `country_code`, `latitude`, `longitude` pour la localisation structurée.', '- Les régions, départements et codes INSEE doivent provenir d’une source de référence versionnée et ne doivent pas être déduits aveuglément du texte.', '', '## Échantillon déterministe', '', "Le CSV associé contient ".count($sample)." enregistrement(s), plafonné à {$limit}; sélection par classification, anomalies puis `legacy_wp_id` croissant."]);
        return implode("\n", $lines)."\n";
    }

    private function writeCsv(string $path, array $sample): void { $out = fopen($path, 'w'); fputcsv($out, ['v2_id','legacy_wp_id','status','name','address','stored_postal_code','stored_city_name','derived_postal_code','derived_city_name','latitude','longitude','classification','issues','legacy_published_at']); foreach ($sample as $r) fputcsv($out, [$r->id,$r->legacy_wp_id,$r->status,$r->name,$r->address,$r->postal_code,$r->city_name,$r->derived_postal_code,$r->derived_city_name,$r->latitude,$r->longitude,$r->classification,implode(' | ', $r->issues),$r->legacy_published_at]); fclose($out); }
    private function present(?string $value): bool { return trim((string) $value) !== ''; }
    /** Extract only an end-of-line French/DOM postcode-city suffix; never persists the result. */
    private function extractAddressParts(string $address): array
    {
        if (! preg_match('/\\b((?:0[1-9]|[1-8]\\d|9[0-5]|97[1-8]|98[0-8])\\d{3})\\s+([^,;\\n]+)\\s*$/u', trim($address), $m)) return ['postal_code' => null, 'city_name' => null];
        $city = trim($m[2]);
        return ['postal_code' => $m[1], 'city_name' => $city !== '' && mb_strlen($city) <= 100 ? $city : null];
    }

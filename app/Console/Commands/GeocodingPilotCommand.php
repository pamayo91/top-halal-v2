...
class GeocodingPilotCommand extends Command
{
    ...
    {
        return DB::table('restaurants')->orderBy('legacy_wp_id')->get()->map(function ($r) {
            [$r->derived_postal, $r->derived_city] = $this->parts((string) $r->address); $gps = $r->latitude !== null && $r->longitude !== null;
            $complete = $r->address && $r->derived_postal && $r->derived_city;
            $r->class = $complete ? ($gps ? 'COMPLETE_WITH_GPS' : 'COMPLETE_NO_GPS') : ($gps ? 'PARTIAL_WITH_GPS' : 'PARTIAL_NO_GPS');

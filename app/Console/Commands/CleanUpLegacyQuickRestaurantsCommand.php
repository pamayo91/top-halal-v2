...
class CleanUpLegacyQuickRestaurantsCommand extends Command {
 ...
 private function redirectObsolete():int { $current=Restaurant::query()->whereNull('legacy_wp_id')->where('slug','like','quick-%')->get();$rows=Restaurant::onlyTrashed()->whereNotNull('legacy_wp_id')->whereRaw("LOWER(name) REGEXP '(^| )quick( |$)'")->get();$created=0;$replaced=0;foreach($rows as $row){$targets=$current->filter(fn($r)=>$this->n($r->postal_code)===$this->n($row->postal_code)&&$this->n($r->city_name)===$this->n($row->city_name));$target=$targets->count()===1?$targets->first():null;if($this->option('apply')){RedirectRule::updateOrCreate(['source_path'=>'/resto/'.$row->slug,'match_type'=>'exact','query_pattern'=>null],['destination'=>$target?'/resto/'.$target->slug:'/quick-hallal','status_code'=>301,'preserve_query'=>false,'priority'=>100,'is_active'=>true,'origin'=>'quick_cleanup','source_rule'=>$target?'Official Quick replacement':'Official Quick obsolete fallback']);}$target?$replaced++:$created++;}$this->info(json_encode(['obsolete_redirects'=>$created,'replacement_redirects'=>$replaced,'candidates'=>$rows->count()]));return self::SUCCESS;}
}

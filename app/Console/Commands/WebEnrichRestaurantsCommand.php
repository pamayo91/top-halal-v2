...
class WebEnrichRestaurantsCommand extends Command
{
    ...
    private function claim(Restaurant $r): bool { $audit=RestaurantWebEnrichment::firstOrCreate(['restaurant_id'=>$r->id],['legacy_wp_id'=>$r->legacy_wp_id]); $allowed=$this->option('retry-errors')?['ERROR','PROCESSING']:['PENDING']; return RestaurantWebEnrichment::whereKey($audit->id)->whereIn('status',$allowed)->update(['status'=>'PROCESSING','processing_started_at'=>now(),'attempts'=>$audit->attempts+1])===1; }
    private function row(Restaurant $r,array $x): array { return ['restaurant_id'=>$r->id,'legacy_wp_id'=>$r->legacy_wp_id,'name'=>$r->name,'address'=>implode(', ',array_filter([$r->address_line1?:$r->address,$r->postal_code,$r->city_name])),'matching'=>json_encode($x['matching']??[]),'activity_status'=>$x['activity_status']??'','status'=>$x['status'],'reason'=>$x['reason']??'','sources'=>json_encode($x['sources']??[],JSON_UNESCAPED_UNICODE),'hours_before'=>json_encode($x['hours_before']??[]),'hours_found'=>json_encode($x['hours_after']??[]),'hours_applied'=>($x['hours_after']??null)?'yes':'no','description_before'=>$x['description_before']??'','description_after'=>$x['description_after']??'','description_applied'=>($x['description_after']??null)?'yes':'no','confidence'=>$x['confidence']??'']; }
    private function report(array $rows): void { $dir=base_path($this->option('out')); File::ensureDirectoryExists($dir); $path=$dir.'/batch-'.now()->format('Ymd-His').'.csv'; $out=fopen($path,'w'); fputcsv($out,['restaurant_id','legacy_wp_id','name','address','matching','activity_status','status','reason','sources','hours_before','hours_found','hours_applied','description_before','description_after','description_applied','confidence']); foreach($rows as $row)fputcsv($out,$row); fclose($out); $this->info('Rapport CSV : '.$path); foreach(collect($rows)->countBy('status')->sortKeys() as $status=>$count)$this->line($status.' : '.$count); }
}

...
class WebEnrichRestaurantsCommand extends Command
{
    ...
    private function candidates()
    {
        ...
        elseif ($ids=$this->ids()) $q->whereIn('restaurants.id', $ids);
    ...
    {
        return array_values(array_unique(array_filter(array_map('intval', explode(',', (string) $this->option('ids'))), fn (int $id) => $id > 0)));
    }
    ...
    private function row(Restaurant $r,array $x): array { $audit=RestaurantWebEnrichment::where('restaurant_id',$r->id)->first(); return ['restaurant_id'=>$r->id,'legacy_wp_id'=>$r->legacy_wp_id,'name'=>$r->name,'address'=>implode(', ',array_filter([$r->address_line1?:$r->address,$r->postal_code,$r->city_name])),'previous_status'=>$audit?->previous_status,'matching'=>json_encode($x['matching']??[]),'activity_status'=>$x['activity_status']??'','status'=>$x['status'],'reason'=>$x['reason']??'','research_count'=>$x['research_count']??0,'search_queries'=>json_encode($x['search_queries']??[],JSON_UNESCAPED_UNICODE),'sources'=>json_encode($x['sources']??[],JSON_UNESCAPED_UNICODE),'rejected_sources'=>json_encode($x['rejected_sources']??[],JSON_UNESCAPED_UNICODE),'hours_before'=>json_encode($x['hours_before']??[]),'hours_found'=>json_encode($x['hours_after']??[]),'hours_applied'=>($x['hours_after']??null)?'yes':'no','description_before'=>$x['description_before']??'','description_after'=>$x['description_after']??'','description_applied'=>($x['description_after']??null)?'yes':'no','confidence_level'=>$x['confidence_level']??'','matching_confidence'=>$x['matching_confidence']??'','activity_confidence'=>$x['activity_confidence']??'','hours_confidence'=>$x['hours_confidence']??'','description_confidence'=>$x['description_confidence']??'']; }
    private function report(array $rows): void { $dir=base_path($this->option('out')); File::ensureDirectoryExists($dir); $path=$dir.'/batch-'.now()->format('Ymd-His').'.csv'; $out=fopen($path,'w'); fputcsv($out,['restaurant_id','legacy_wp_id','name','address','previous_status','matching','activity_status','status','reason','research_count','search_queries','sources','rejected_sources','hours_before','hours_found','hours_applied','description_before','description_after','description_applied','confidence_level','matching_confidence','activity_confidence','hours_confidence','description_confidence']); foreach($rows as $row)fputcsv($out,$row); fclose($out); $this->info('Rapport CSV : '.$path); foreach(collect($rows)->countBy('status')->sortKeys() as $status=>$count)$this->line($status.' : '.$count); }
}

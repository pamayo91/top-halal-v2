...
class CitySeoService {
 private const KEY='city-seo-stats-v2';
 ...
 public function cities():Collection{return collect(Cache::rememberForever(self::KEY,function(){ $configs=CitySeoPage::all()->keyBy('city_name');$threshold=$this->threshold();return Restaurant::where('status','published')->whereNotNull('city_name')->where('city_name','!=','')->selectRaw('city_name,count(*) restaurants_count')->groupBy('city_name')->orderByDesc('restaurants_count')->orderBy('city_name')->get()->map(function($row)use($configs,$threshold){$config=$configs->get($row->city_name);$state=$config?->state??'auto';$open=$state==='forced_open'||($state!=='forced_closed'&&$row->restaurants_count>=$threshold);return ['city_name'=>$row->city_name,'slug'=>Str::slug($row->city_name),'restaurants_count'=>(int)$row->restaurants_count,'config'=>$config?->getAttributes(),'open'=>$open];})->all();}))->map(fn($city)=>(object)[...$city,'config'=>isset($city['config'])?new CitySeoPage($city['config']):null]);}

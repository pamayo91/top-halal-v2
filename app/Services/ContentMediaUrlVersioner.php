...
class ContentMediaUrlVersioner
{
    ...
    {
        preg_match_all('#/media/(\d+)(?!\d)(?:/(\d+)(?!\d))?(?!/v/)#', $html, $matches, PREG_SET_ORDER);
        ...
        $replaced = 0;
        $unresolved = [];
        $rewritten = preg_replace_callback('#/media/(\d+)(?!\d)(?:/(\d+)(?!\d))?(?!/v/)#', function (array $match) use ($assets, &$replaced, &$unresolved): string {

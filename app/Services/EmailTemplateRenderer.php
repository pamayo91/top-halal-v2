...
class EmailTemplateRenderer
{
    ...
    public function render(string $key, array $values): array
    {
        ...
        return ['subject' => $replace($override?->subject ?? $default['subject']), 'body' => $replace($override?->body ?? $default['body']), 'cta_label' => $replace($override?->cta_label ?? $default['cta_label']), 'cta_url' => $safe['action_url'] ?? $safe['verification_url'] ?? $safe['reset_url'] ?? null];
    }

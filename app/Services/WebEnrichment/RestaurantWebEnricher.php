...
class RestaurantWebEnricher
{
    ...
    public function process(Restaurant $restaurant, bool $dryRun = false): array
    {
        ...
        if (($evidence['state'] ?? null) !== 'matched') return $this->finish($audit, $base + ['status'=>'INSUFFICIENT_DATA','reason'=>$evidence['reason'] ?? 'No sufficiently reliable name and address match'], $dryRun);
        if (($evidence['closure'] ?? null) === 'confirmed') return $this->finish($audit, $base + ['status'=>'CLOSED_CONFIRMED_REVIEW','reason'=>'Matched establishment is permanently closed or the matching company is deregistered','closure_sources'=>$evidence['closure_sources'] ?? [],'confidence'=>$evidence['confidence'] ?? null], $dryRun);
        ...
        if ($hours === [] && $description === null) return $this->finish($audit, $base + ['status'=>'UNCHANGED','reason'=>'No safe eligible enrichment','confidence'=>$evidence['confidence'] ?? null], $dryRun);
        $changes = $base + ['status'=>'UPDATED','reason'=>null,'confidence'=>$evidence['confidence'] ?? null,'hours_after'=>$hours ?: null,'description_after'=>$description,'hours_source'=>$hours ? ($evidence['hours_source'] ?? null) : null,'description_sources'=>$description ? ($evidence['description_sources'] ?? []) : []];

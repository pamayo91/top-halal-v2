...
class ContentTransformer
{
    public function transform(string $html): array
    {
        ...
        $html = preg_replace_callback('/\[caption\b[^\]]*\](.*?)\[\/caption\]/is', function (array $match): string { $body = trim($match[1]); if (!preg_match('/<img\b[^>]*>/i', $body, $image, PREG_OFFSET_CAPTURE)) return '<figure class="legacy-caption">'.$body.'</figure>'; $end = $image[0][1] + strlen($image[0][0]); $media = substr($body, 0, $end); $tail = preg_replace('/^\s*<\/a>/', '', substr($body, $end)); return '<figure class="legacy-caption">'.$media.(trim($tail) !== '' ? '<figcaption>'.trim($tail).'</figcaption>' : '').'</figure>'; }, $html);

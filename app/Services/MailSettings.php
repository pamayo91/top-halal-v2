...
class MailSettings
{
    ...
    {
        $v = $this->values(); $mailer = 'smtp';
        if (blank($v['host'] ?? null)) return $mailer;
    ...
    {
        $current = $this->values(); $input['mailer'] = 'smtp'; unset($input['admin_email']); if (blank($input['password'] ?? null)) $input['password'] = $current['password'] ?? null; else $input['password'] = Crypt::encryptString($input['password']);

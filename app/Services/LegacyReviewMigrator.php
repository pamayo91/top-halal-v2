...
class LegacyReviewMigrator
{
    ...
    public function inspect(object $post): array
    {
        ...
        return [
            ...
            'target' => ['restaurant_id' => $restaurant?->id, 'legacy_wp_review_id' => (int) $post->ID, 'legacy_user_id' => (int) $post->post_author ?: null, 'author_name' => app(TextNormalizer::class)->plainText(trim((string) $author) ?: 'Anonyme'), 'author_email' => null, 'rating' => is_numeric($rating) ? (int) $rating : null, 'title' => app(TextNormalizer::class)->plainText(trim((string) $post->post_title)) ?: null, 'content' => app(TextNormalizer::class)->plainText(trim(strip_tags((string) $post->post_content))), 'status' => $post->post_status === 'publish' ? 'approved' : 'pending', 'approved_at' => $post->post_status === 'publish' && $post->post_date_gmt !== '0000-00-00 00:00:00' ? $post->post_date_gmt : null, 'created_at' => $this->historicalDate($post), 'updated_at' => $post->post_modified_gmt === '0000-00-00 00:00:00' ? now() : $post->post_modified_gmt],

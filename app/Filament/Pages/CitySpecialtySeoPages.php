...
class CitySpecialtySeoPages extends Page implements HasTable
{
    ...
    public function table(Table $table): Table
    {
        return $table->query($this->facetQuery())->columns([
            ...
            BadgeColumn::make('facet_state')->label('État')->state(fn (Restaurant $record): string => $record->facet_state === 'open' ? 'Ouverte' : 'Fermée')->color(fn (string $state): string => $state === 'Ouverte' ? 'success' : 'gray')->url(fn (Restaurant $record): ?string => $record->facet_state === 'open' ? $this->publicUrl($record) : null)->openUrlInNewTab(),
    ...
    private function facetQuery(): Builder
    {
        ...
        return Restaurant::query()
            ...
            ->selectRaw("MIN(restaurants.id) as id, MIN(restaurants.city_name) as city_name, {$cityCode} as city_code, categories.id as category_id, categories.name as category_name, categories.slug as category_slug, COUNT(*) as restaurants_count, city_specialty_seo_pages.state as facet_state, city_specialty_seo_pages.content_top, city_specialty_seo_pages.content_bottom")
            ->groupByRaw("{$cityCode}, categories.id, categories.name, categories.slug, city_specialty_seo_pages.state, city_specialty_seo_pages.content_top, city_specialty_seo_pages.content_bottom");
    }

    private function defaultFormData(): array { return ['state' => 'closed', 'h1' => '', 'seo_title' => '', 'seo_description' => '', 'content_top' => '', 'content_bottom' => '']; }
    private function publicUrl(Restaurant $record): ?string { $city = app(CityPageResolver::class)->cityForCode($record->city_code); return $city === null ? null : route('city-specialties.show', ['city' => $city->slug, 'specialty' => $record->category_slug]); }

...
class CitySpecialtySeoPages extends Page implements HasTable
{
    ...
    public function table(Table $table): Table
    {
        ...
        return $table->query($this->facetQuery())->columns([
            ...
            BadgeColumn::make('facet_state')->label('État')->state(fn (Restaurant $r): string => $r->facet_state === 'open' ? 'Ouverte' : 'Fermée')->color(fn (string $s): string => $s === 'Ouverte' ? 'success' : 'gray')->url(fn (Restaurant $r): ?string => $r->facet_state === 'open' ? $this->publicUrl($r) : null)->openUrlInNewTab(),
            BadgeColumn::make('has_content')->label('Contenu personnalisé')->state(fn (Restaurant $r): string => filled($r->content_top) || filled($r->content_bottom) ? 'Oui' : 'Non')->color(fn (string $s): string => $s === 'Oui' ? 'success' : 'gray'),
        ])->filters([
            SelectFilter::make('term')->label($label)->options(fn (): array => $this->termModel()::query()->orderBy('name')->pluck('name', 'id')->all())->query(fn (Builder $q, array $d): Builder => filled($d['value'] ?? null) ? $q->where($terms.'.id', $d['value']) : $q),
            SelectFilter::make('state')->label('État')->options(['open' => 'Ouverte', 'closed' => 'Fermée'])->query(fn (Builder $q, array $d): Builder => match ($d['value'] ?? null) {'open' => $q->where($config.'.state', 'open'), 'closed' => $q->where(fn (Builder $s) => $s->whereNull($config.'.state')->orWhere($config.'.state', 'closed')), default => $q}),

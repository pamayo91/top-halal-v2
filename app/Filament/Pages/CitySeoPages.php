...
use App\Services\{AdminAudit, CityPageResolver, CitySeoService, ContentSanitizer};
...
class CitySeoPages extends Page implements HasTable
{
    ...
    public function table(Table $table): Table
    {
        return $table->query($this->cityQuery())->columns([
            TextColumn::make('city_name')->label('Ville')->state(fn (Restaurant $record): string => $this->cityLabel($record))
                ->searchable(query: fn (Builder $query, string $search): Builder => $query->where('restaurants.city_name', 'like', '%'.$search.'%'))
                ->sortable(query: fn (Builder $query, string $direction): Builder => $query->orderByRaw('MIN(restaurants.city_name) '.$direction)),
            TextColumn::make('city_slug')->label('Slug')->state(fn (Restaurant $record) => app(CityPageResolver::class)->cityForCode($record->city_code)?->slug ?? Str::slug($record->city_name))->sortable(),
        ...
        ])->recordActions([Action::make('edit')->label('Modifier')->url(fn (Restaurant $record): string => static::getUrl(['city' => $record->city_code]).'#city-seo-editor')])
    ...
    private function cityQuery(): Builder
    {
        ...
        return Restaurant::query()->where('restaurants.status', 'published')->whereNotNull('restaurants.city_name')->where('restaurants.city_name', '!=', '')->whereNotNull('restaurants.city_code')->where('restaurants.city_code', '!=', '')
            ->leftJoin('city_seo_pages', fn (JoinClause $join) => $join->on('city_seo_pages.city_code', '=', DB::raw($cityCode)))
            ->selectRaw("MIN(restaurants.id) as id, MIN(restaurants.city_name) as city_name, {$cityCode} as city_code, COUNT(*) as restaurants_count, city_seo_pages.state as city_state, city_seo_pages.content_top, city_seo_pages.content_bottom")
            ->groupByRaw("{$cityCode}, city_seo_pages.state, city_seo_pages.content_top, city_seo_pages.content_bottom");
    }
    ...
    public function selectCity(string $city): void { $page=app(CityPageResolver::class)->cityForCode($city);if($page===null)return;$this->cityCode=$page->city_code;$this->cityName=$page->city_name;$config=CitySeoPage::firstWhere('city_code',$this->cityCode);$this->form->fill([...$this->defaultFormData(),'state'=>$config?->state??'auto','h1'=>$config?->h1??'','seo_title'=>$config?->seo_title??'','seo_description'=>$config?->seo_description??'','content_top'=>$config?->content_top??'','content_bottom'=>$config?->content_bottom??'']); }
    public function save(): void { $data=$this->form->getState();Setting::updateOrCreate(['key'=>'city_seo_minimum_restaurants'],['value'=>['value'=>(int)$data['threshold']],'group'=>'seo']);if($this->cityCode!==''){foreach(['content_top','content_bottom'] as $key)$data[$key]=app(ContentSanitizer::class)->sanitize($data[$key]??'')['html'];unset($data['threshold']);CitySeoPage::updateOrCreate(['city_code'=>$this->cityCode],['city_name'=>$this->cityName,...$data]);}app(CitySeoService::class)->forget();app(AdminAudit::class)->record('city_seo.updated','city_seo',['city_code' => $this->cityCode, 'city_name' => $this->cityName]);Notification::make()->title('Réglages enregistrés')->success()->send();$this->resetTable(); }
    ...
    private function cityLabel(Restaurant $record): string { $city=app(CityPageResolver::class)->cityForCode($record->city_code);return $city===null ? $record->city_name : $city->city_name.($city->is_ambiguous ? ' — '.$city->department['name'] : ''); }

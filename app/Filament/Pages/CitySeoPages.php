...
class CitySeoPages extends Page implements HasTable
{
    ...
    public function selectCity(string $city): void { $this->cityName=$city;$config=CitySeoPage::firstWhere('city_name',$city);$this->form->fill(['threshold'=>app(CitySeoService::class)->threshold(),'state'=>$config?->state??'auto','h1'=>$config?->h1,'seo_title'=>$config?->seo_title,'seo_description'=>$config?->seo_description,'content_top'=>$config?->content_top,'content_bottom'=>$config?->content_bottom]); }

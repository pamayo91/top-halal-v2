...
class RestaurantReviewResource extends AdminResource
{
    ...
    public static function getEloquentQuery(): Builder { return parent::getEloquentQuery()->with('restaurant'); }
    public static function form(Schema $schema): Schema { return $schema->components([TextInput::make('created_at')->disabled()->dehydrated(false), Textarea::make('content')->disabled()->columnSpanFull(), Textarea::make('moderation_note')->label('Note interne')->dehydrated(false)->columnSpanFull()]); }
    ...
    public static function table(Table $table): Table
    {
        return $table->columns([
            ...
            ->recordActions([static::viewOnSiteAction(), Action::make('approve')->label('Approuver')->color('success')->visible(fn (RestaurantReview $r) => $r->status === 'pending')->action(fn (RestaurantReview $r) => static::moderate($r, 'approved')), Action::make('reject')->label('Refuser')->color('danger')->requiresConfirmation()->visible(fn (RestaurantReview $r) => $r->status === 'pending')->action(fn (RestaurantReview $r) => static::moderate($r, 'rejected')), Action::make('spam')->label('Marquer comme spam')->color('danger')->requiresConfirmation()->visible(fn (RestaurantReview $r) => $r->status === 'pending')->action(fn (RestaurantReview $r) => static::moderate($r, 'spam')), EditAction::make()->label('Voir')])
            ->toolbarActions([BulkActionGroup::make([BulkAction::make('approve')->label('Approuver la sélection')->color('success')->requiresConfirmation()->action(fn ($records) => $records->each(fn (RestaurantReview $r) => static::moderate($r, 'approved'))), BulkAction::make('reject')->label('Refuser la sélection')->color('danger')->requiresConfirmation()->action(fn ($records) => $records->each(fn (RestaurantReview $r) => static::moderate($r, 'rejected'))), BulkAction::make('spam')->label('Marquer la sélection comme spam')->color('danger')->requiresConfirmation()->action(fn ($records) => $records->each(fn (RestaurantReview $r) => static::moderate($r, 'spam')))])])
            ->defaultSort('created_at', 'desc')->emptyStateHeading('Aucun avis dans cette file');
    }

...
class RestaurantReviewResource extends AdminResource
{
 ...
 public static function form(Schema $schema):Schema{return $schema->components([TextInput::make('created_at')->label('Date historique de l’avis')->disabled()->dehydrated(false)->formatStateUsing(fn($state)=>$state ? \Illuminate\Support\Carbon::parse($state)->locale('fr')->translatedFormat('d F Y à H:i') : '—'),TextInput::make('approved_at')->label('Date de modération V2')->disabled()->dehydrated(false)->formatStateUsing(fn($state)=>$state ? \Illuminate\Support\Carbon::parse($state)->locale('fr')->translatedFormat('d F Y à H:i') : '—'),Textarea::make('content')->disabled()->columnSpanFull(),Textarea::make('moderation_note')->label('Note interne')->dehydrated(false)->columnSpanFull()]);}
 public static function table(Table $table):Table{return $table->columns([TextColumn::make('restaurant.name')->label('Restaurant')->searchable()->sortable(),TextColumn::make('author_name')->label('Auteur')->searchable()->description(fn(RestaurantReview $r)=>str($r->content)->limit(110)),TextColumn::make('rating')->label('Note')->suffix('/5')->sortable(),TextColumn::make('status')->badge()->color(fn(string $state)=>$state==='approved'?'success':($state==='pending'?'warning':'danger'))->sortable(),TextColumn::make('created_at')->label('Avis le')->dateTime('d/m/Y H:i')->sortable(),TextColumn::make('approved_at')->label('Modéré le')->dateTime('d/m/Y H:i')->placeholder('—')->sortable()->toggleable()])->filters([SelectFilter::make('status')->default('pending')->options(['pending'=>'En attente','approved'=>'Approuvé','rejected'=>'Refusé','spam'=>'Spam'])])->recordActions([Action::make('approve')->label('Approuver')->color('success')->visible(fn(RestaurantReview $r)=>$r->status==='pending')->action(fn(RestaurantReview $r)=>static::moderate($r,'approved')),Action::make('reject')->label('Refuser')->color('danger')->requiresConfirmation()->visible(fn(RestaurantReview $r)=>$r->status==='pending')->action(fn(RestaurantReview $r)=>static::moderate($r,'rejected')),EditAction::make()->label('Voir')])->toolbarActions([BulkActionGroup::make([BulkAction::make('approve')->label('Approuver la sélection')->requiresConfirmation()->action(fn($records)=>$records->each(fn(RestaurantReview $r)=>static::moderate($r,'approved'))),BulkAction::make('reject')->label('Refuser la sélection')->requiresConfirmation()->action(fn($records)=>$records->each(fn(RestaurantReview $r)=>static::moderate($r,'rejected')))])])->defaultSort('created_at','desc')->emptyStateHeading('Aucun avis dans cette file');}

...
class RestaurantReviewResource extends AdminResource
{
 ...
 public static function form(Schema $schema):Schema{return $schema->components([TextInput::make('created_at')->label('Date historique de l’avis')->disabled()->dehydrated(false)->formatStateUsing(fn($state)=>$state?->translatedFormat('d F Y à H:i')),TextInput::make('approved_at')->label('Date de modération V2')->disabled()->dehydrated(false)->formatStateUsing(fn($state)=>$state?->translatedFormat('d F Y à H:i')??'—'),Textarea::make('content')->disabled()->columnSpanFull(),Textarea::make('moderation_note')->label('Note interne')->dehydrated(false)->columnSpanFull()]);}

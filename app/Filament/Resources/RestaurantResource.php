...
class RestaurantResource extends AdminResource
{
    ...
    public static function previewAction(): Action
    {
        return Action::make('preview')
            ...
            ->openUrlInNewTab()
            ->visible(fn (Restaurant $restaurant): bool => ! $restaurant->trashed() && $restaurant->status === 'pending' && $restaurant->submission?->status !== 'rejected');
    }
        ...
        return $schema->components([Tabs::make('Restaurant')->tabs([
            ...
            Tabs\Tab::make('Proposition')->visible(fn (?Restaurant $record): bool => $record?->submission !== null)->schema([
                Section::make('Décision de modération')->schema([
                    ...
                    TextInput::make('submission_rejection_reference')->label('Référence de contestation')->readOnly()->dehydrated(false)->visible(fn (?Restaurant $record): bool => $record?->submission?->status === 'rejected')->formatStateUsing(fn (?Restaurant $record): string => $record?->submission?->rejection_reference ?: '—'),
                ]),
            ]),
    ...
    public static function table(Table $table): Table
    {
        return $table->columns([
            ...
            TextColumn::make('submission.status')->label('Étape proposition')->badge()->formatStateUsing(fn (?string $state): string => match ($state) {'pending_email_verification' => 'E-mail à confirmer', 'pending_admin_review' => 'À vérifier par Top Halal', 'published' => 'Publiée', 'rejected' => 'Refusée', default => '—'})->color(fn (?string $state): string => match ($state) {'pending_email_verification' => 'warning', 'pending_admin_review' => 'info', 'published' => 'success', 'rejected' => 'danger', default => 'gray'})->toggleable(),
            TextColumn::make('submission.duplicate_signal')->label('Doublon')->badge()->formatStateUsing(fn (?string $state): string => $state === 'potential' ? 'À vérifier' : '—')->color(fn (?string $state): string => $state === 'potential' ? 'warning' : 'gray')->description(fn (Restaurant $r): ?string => $r->submission?->duplicate_signal === 'potential' ? count($r->submission->duplicate_details ?? []).' rapprochement(s) potentiel(s)' : null)->toggleable(),
            ...
            SelectFilter::make('submission_status')->label('Étape proposition')->options(['pending_email_verification' => 'E-mail à confirmer', 'pending_admin_review' => 'À vérifier par Top Halal', 'published' => 'Publiée', 'rejected' => 'Refusée'])->query(fn (Builder $query, array $data): Builder => $query->when($data['value'] ?? null, fn (Builder $query, string $state): Builder => $query->whereHas('submission', fn (Builder $submission): Builder => $submission->where('status', $state)))),
            Filter::make('potential_duplicate')->label('Doublon potentiel')->query(fn (Builder $query): Builder => $query->whereHas('submission', fn (Builder $submission): Builder => $submission->where('duplicate_signal', 'potential'))),
        ...
        ])->recordActions([static::viewOnSiteAction()->visible(fn (Restaurant $restaurant) => ! $restaurant->trashed() && $restaurant->status === 'published'), static::previewAction(), Action::make('publish_submission')->label('Publier')->color('success')->requiresConfirmation()->visible(fn (Restaurant $restaurant): bool => ! $restaurant->trashed() && $restaurant->status === 'pending' && $restaurant->submission?->status === 'pending_admin_review')->action(function (Restaurant $restaurant): void {$restaurant->update(['status' => 'published']); app(AdminAudit::class)->record('restaurant.published', $restaurant);}), Action::make('reject_submission')->label('Refuser')->icon('heroicon-o-x-circle')->color('danger')->requiresConfirmation()->modalHeading('Refuser cette proposition ?')->modalDescription('Le restaurant restera conservé hors de la partie publique pour l’historique et l’audit. Cette décision ne peut pas être annulée depuis cette action.')->modalSubmitActionLabel('Refuser la proposition')->form([Textarea::make('admin_rejection_reason')->label('Motif de refus')->rows(4)->maxLength(5000)->helperText('Facultatif. Il sera communiqué au déposant s’il est renseigné.')])->visible(fn (Restaurant $restaurant): bool => ! $restaurant->trashed() && $restaurant->status === 'pending' && $restaurant->submission?->status === 'pending_admin_review')->action(fn (Restaurant $restaurant, array $data): mixed => app(RestaurantSubmissionModeration::class)->reject($restaurant, $data['admin_rejection_reason'] ?? null)), EditAction::make()->visible(fn (Restaurant $restaurant) => ! $restaurant->trashed()), static::trashAction(), static::restoreAction(), static::forceDeleteAction()])

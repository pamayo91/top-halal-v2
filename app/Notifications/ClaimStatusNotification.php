...
class ClaimStatusNotification extends Notification implements ShouldQueue
{
    ...
    public function toMail(object $notifiable): TemplateMailable
    {
        ...
        return (new TemplateMailable($key,['site_name'=>config('app.name','Top Halal'),'user_name'=>$notifiable->name,'restaurant_name'=>$restaurant,'action_url'=>$url,'claim_note'=>$this->claim->admin_note ?: 'Vous pouvez contacter la modération avec les justificatifs nécessaires.']))->to($notifiable->routeNotificationForMail());
    }
}

...
<section id="avis" class="section reviews"><h2>Avis</h2><p data-review-aggregate>{{ $aggregate['count'] }} avis{{ $aggregate['average'] !== null ? ', moyenne '.number_format($aggregate['average'], 2, ',', '').'/5' : '' }}</p>@if(session('review_submitted'))<p class="flash" role="status">Merci, votre avis sera publié après modération.</p>@elseif(session('contribution_verification_sent'))<p class="flash" role="status">Vérifiez votre adresse e-mail pour envoyer votre avis à la modération.</p>@endif @forelse($reviews as $review)<article class="review"><p class="rating">{{ str_repeat('★', $review->rating) }}<span class="sr-only">{{ $review->rating }} sur 5</span></p><h3>{{ $review->title }}</h3><p>{{ $review->content }}</p><p class="muted">{{ $review->author_name }} · <time datetime="{{ $review->created_at->toDateString() }}">Publié le {{ $review->created_at->translatedFormat('j F Y') }}</time></p></article>@empty <p>Aucun avis publié pour le moment.</p>@endforelse
@if(! $isPreview)@if(auth()->user()?->can('isRestaurantManager', $restaurant))<p class="flash" role="status">{{ \App\Exceptions\RestaurantReviewOwnershipException::MESSAGE }}</p>@else<details><summary>Donner mon avis</summary><form class="stack-form" method="post" action="{{ route('restaurants.reviews.store', $restaurant->slug) }}">@csrf<label>Nom <input name="name" required value="{{ old('name', auth()->user()?->name) }}"></label>@guest<label>E-mail <input name="email" type="email" required value="{{ old('email') }}"></label>@endguest<label>Note <select name="rating" required><option value="">Choisir</option>@for($i=5;$i>=1;$i--)<option value="{{ $i }}">{{ $i }}/5</option>@endfor</select></label><label>Titre (facultatif) <input name="title" value="{{ old('title') }}"></label><label>Avis <textarea name="content" required maxlength="3000">{{ old('content') }}</textarea></label><input class="hp" name="website" tabindex="-1" autocomplete="off"><button class="button">Envoyer pour modération</button></form></details>@endif@endif</section>
@if(! $isPreview)

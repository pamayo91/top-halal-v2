        ...
        <div class="border-t border-gray-200 pt-5 dark:border-gray-700">
            ...
            <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                @foreach($fallbacks as $media)
                    @if($asset = $media->asset)
                        @php($variant = $asset->variants->where('width', '>=', 480)->sortBy('width')->first() ?? $asset->variants->sortByDesc('width')->first())
                        <figure class="overflow-hidden rounded-lg border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900"><div class="flex aspect-[4/3] items-center justify-center bg-gray-100 p-2 dark:bg-gray-800"><img src="{{ $asset->deliveryUrl($variant?->width) }}" width="{{ $variant?->width ?? $asset->width }}" height="{{ $variant?->height ?? $asset->height }}" loading="lazy" class="max-h-full max-w-full object-contain" alt="{{ $asset->alt_text ?: $restaurant->name }}"></div><figcaption class="space-y-3 p-3 text-sm text-gray-600 dark:text-gray-300"><div>Miniature de spécialité (sans couverture)</div><button type="button" wire:click="detachFallbackRestaurantMedia({{ $media->id }})" wire:confirm="Retirer cette miniature de spécialité de la fiche ? Elle restera disponible dans la médiathèque et pour les autres restaurants." wire:loading.attr="disabled" class="fi-btn fi-btn-size-sm fi-btn-color-danger">Retirer</button></figcaption></figure>
                    @endif
                @endforeach
            </div>
        </div>

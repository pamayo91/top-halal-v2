<x-filament-panels::page>
    @php($tree = $this->tree())
    <div class="menu-editor" x-data="{ dragging: null }">
    ...
    @if($itemData !== null)
        ...
        <section class="menu-editor__modal" role="dialog" aria-modal="true" aria-labelledby="menu-item-title" x-data x-init="$nextTick(() => $refs.label.focus())" x-on:keydown.escape.window="$wire.closeItemEditor()">
            <form wire:submit="saveItem">
                ...
                @if(in_array($itemData['link_type'], ['internal_url', 'external_url'], true))<label>URL <input type="url" wire:model="itemData.url" placeholder="{{ $itemData['link_type'] === 'internal_url' ? '/restaurants' : 'https://…' }}"></label>@endif

...
@foreach($blocks as $block)
    ...
    @elseif($block['type'] === 'correction')
        <section class="sidebar-card sidebar-correction"><div class="sidebar-heading-icon" aria-hidden="true">✓</div><p class="sidebar-title">{{ $title }}</p>@if(session('content_report_submitted'))<p class="flash" role="status">Merci, votre signalement a été transmis à Top Halal.</p>@elseif(session('content_report_verification_sent'))<p class="flash" role="status">Vérifiez votre adresse e-mail pour transmettre votre signalement.</p>@else<p class="muted">Un détail est à mettre à jour ? Dites-nous lequel.</p><details><summary>Nous le signaler <span aria-hidden="true">→</span></summary><form class="stack-form" method="post" action="{{ route('editorial.reports.store', $content->slug) }}">@csrf@if(auth()->check())<p class="muted">Signalement envoyé avec le compte {{ auth()->user()->email }}.</p>@else<label for="report-email-{{ $placement }}">E-mail <input id="report-email-{{ $placement }}" name="email" type="email" required value="{{ old('email') }}"></label>@endif<label class="sr-only" for="report-{{ $placement }}">Votre signalement</label><textarea id="report-{{ $placement }}" name="message" maxlength="2000" required>{{ old('message') }}</textarea><input class="hp" name="website" tabindex="-1" autocomplete="off"><button class="button button-small">Envoyer</button></form></details>@endif</section>
    ...
    @elseif($block['type'] === 'share')
        <section class="sidebar-card sidebar-share"><p class="sidebar-title">{{ $title ?: ($isArticle ? 'Partager cet article' : 'Partager cette page') }}</p><div class="share-links"><a aria-label="Partager sur Facebook" href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(route('editorial.show', $content->slug)) }}" rel="noopener noreferrer"><span aria-hidden="true">f</span></a><a aria-label="Partager sur X" href="https://twitter.com/intent/tweet?url={{ urlencode(route('editorial.show', $content->slug)) }}" rel="noopener noreferrer"><span aria-hidden="true">𝕏</span></a><a aria-label="Partager par e-mail" href="mailto:?subject={{ rawurlencode($content->title) }}&body={{ urlencode(route('editorial.show', $content->slug)) }}"><svg aria-hidden="true" viewBox="0 0 24 24"><rect x="3" y="5" width="18" height="14" rx="2"/><path d="m4 7 8 6 8-6"/></svg></a></div></section>

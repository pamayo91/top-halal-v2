...
use App\Models\{ContactMessage, EmailDeliveryLog, EmailTemplate, Setting};
...
class EmailContactManagementTest extends TestCase
{
    ...
    public function test_contact_is_persisted_and_queues_admin_and_optional_confirmation(): void
    {
        ...
        $this->post('/contact', ['name' => 'Alice', 'email' => 'alice@top-halal.fr', 'subject' => 'Question', 'message' => '<b>Texte</b>', 'website' => ''])
            ->assertRedirect(route('contact.create'))->assertSessionHas('status', 'Merci');

        $this->assertDatabaseHas('contact_messages', ['name' => 'Alice', 'email' => 'alice@top-halal.fr', 'status' => 'new']);
        $this->assertDatabaseCount('email_delivery_logs', 2);
        Mail::assertQueued(TemplateMailable::class, fn (TemplateMailable $mail) => $mail->templateKey === 'contact_admin' && $mail->replyTo === 'alice@top-halal.fr');
    ...
    {
        Mail::fake();
        Setting::create(['key' => 'contact_settings', 'group' => 'contact', 'value' => ['recipient' => 'admin@top-halal.fr', 'send_confirmation' => false]]);
        $this->post('/contact', ['name' => 'Alice', 'email' => 'alice@top-halal.fr', 'subject' => 'Question', 'message' => 'Texte', 'website' => ''])->assertRedirect();
        $this->assertDatabaseCount('contact_messages', 1); $this->assertDatabaseCount('email_delivery_logs', 1);
        Mail::assertNotQueued(TemplateMailable::class, fn (TemplateMailable $mail) => $mail->templateKey === 'contact_confirmation');
    }

    public function test_contact_validates_honeypot_and_required_fields(): void
    {
        $this->post('/contact', ['website' => 'bot'])->assertSessionHasErrors('message');

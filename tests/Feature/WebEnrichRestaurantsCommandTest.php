...
class WebEnrichRestaurantsCommandTest extends TestCase
{
    ...
    public function test_it_adds_only_missing_hours_and_an_eligible_description_then_skips_the_same_restaurant(): void
    {
        ...
        $audit=RestaurantWebEnrichment::first(); $this->assertSame('  KEBAB FRITES DE ancien texte',$audit->description_before); $this->assertSame($restaurant->fresh()->description,$audit->description_after);
        $csv=collect(File::files(base_path('docs/generated/test-web-enrichment')))->sortByDesc(fn($file)=>$file->getMTime())->first(); $rows=array_map('str_getcsv', file($csv->getPathname())); $report=array_combine($rows[0],$rows[1]); $this->assertSame($audit->description_before,$report['description_before']); $this->assertSame($audit->description_after,$report['description_after']); $this->assertSame('yes',$report['description_applied']);

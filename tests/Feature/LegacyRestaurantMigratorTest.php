...
class LegacyRestaurantMigratorTest extends TestCase
{
    ...
    private function seedRestaurant(int $id, string $title, string $slug, string $status, bool $complete, bool $claimed): void
    {
        ...
        $legacy->table('wp_posts')->insert(['ID' => $id, 'post_author' => 7, 'post_date_gmt' => '2025-01-01 12:00:00', 'post_modified_gmt' => '2025-02-01 12:00:00', 'post_content' => 'Cuisine halal [vc_row]propre[/vc_row]', 'post_title' => $title, 'post_status' => $status, 'post_name' => $slug, 'post_type' => 'listing', 'guid' => '', 'post_excerpt' => '']);
        ...
        if ($complete) { $meta[] = ['post_id' => $id, 'meta_key' => 'gallery_image_ids', 'meta_value' => '900']; $legacy->table('wp_posts')->insert(['ID' => 900, 'post_author' => 0, 'post_date_gmt' => '2025-01-01 12:00:00', 'post_modified_gmt' => '2025-01-01 12:00:00', 'post_content' => '', 'post_title' => '', 'post_status' => 'inherit', 'post_name' => '', 'post_type' => 'attachment', 'guid' => 'https://legacy.test/uploads/image.jpg', 'post_excerpt' => 'Photo été']); }
        if ($claimed) $meta[] = ['post_id' => $id, 'meta_key' => 'claimed', 'meta_value' => '1'];
        $legacy->table('wp_postmeta')->insert($meta);
        foreach ([[1, 'Marocain', 'marocain', 'listing-category'], [2, 'Libanais', 'libanais', 'listing-category'], [3, 'Terrasse', 'terrasse', 'features'], [4, 'À emporter', 'a-emporter', 'features'], [5, 'Paris', 'paris', 'location']] as [$termId, $name, $termSlug, $taxonomy]) { $legacy->table('wp_terms')->insert(['term_id' => $termId, 'name' => $name, 'slug' => $termSlug]); $legacy->table('wp_term_taxonomy')->insert(['term_taxonomy_id' => $termId, 'term_id' => $termId, 'taxonomy' => $taxonomy, 'parent' => 0]); $legacy->table('wp_term_relationships')->insert(['object_id' => $id, 'term_taxonomy_id' => $termId]); }
    }
}

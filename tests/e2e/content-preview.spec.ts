...
test('public comment form requires identity verification and rejects URLs', async ({ page }) => {
  ...
  const commentForm = page.locator('.comments form');
  await commentForm.locator('input[name="name"]').fill('Codex test');
  await commentForm.locator('input[name="email"]').fill('codex-comment-test@example.invalid');
  await commentForm.locator('textarea[name="content"]').fill('Commentaire de validation sans lien.');
  await commentForm.getByRole('button', { name: 'Envoyer pour modération' }).click();
  await expect(page.getByRole('status')).toContainText('Vérifiez votre adresse e-mail');
  await page.getByText('Laisser un commentaire', { exact: true }).click();
  await commentForm.locator('input[name="name"]').fill('Codex test');
  await commentForm.locator('input[name="email"]').fill('codex-comment-test@example.invalid');
  await commentForm.locator('textarea[name="content"]').fill('https://example.invalid');
  await commentForm.getByRole('button', { name: 'Envoyer pour modération' }).click();

...
test('restaurant review preview requires identity verification and rejects URLs', async ({ page }) => {
  ...
  await page.locator('input[name="name"]').fill('Codex review'); await page.locator('input[name="email"]').fill('codex-review-test@example.invalid'); await page.locator('select[name="rating"]').selectOption('5'); await page.locator('textarea[name="content"]').fill('Validation sans lien.'); await page.getByRole('button',{name:'Envoyer'}).click(); await expect(page.getByRole('status')).toContainText('Vérifiez votre adresse e-mail');

...
test('public restaurant review uses accessible stars, requires identity verification and rejects URLs', async ({ page }) => {
  ...
  for (const [rating, label] of [['1', 'Décevant'], ['2', 'Moyen'], ['3', 'Bien'], ['4', 'Très bien'], ['5', 'Excellent']]) { await reviewForm.locator(`input[name="rating"][value="${rating}"]`).check(); await expect(reviewForm.locator(`input[name="rating"][value="${rating}"]`)).toBeChecked(); await expect(reviewForm.getByText(label, { exact: true })).toBeVisible(); }

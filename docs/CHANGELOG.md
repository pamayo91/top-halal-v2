...
- Restaurants : le sélecteur partagé de photos est désormais utilisé par la proposition publique et l’éditeur restaurateur. Les ajouts successifs se cumulent côté navigateur, les nouvelles images s’affichent immédiatement avec badge `Nouvelle`, nom et retrait avant sauvegarde, puis le `FileList` multipart est reconstruit sans upload anticipé. Les contrôles visibles reprennent exactement JPEG/PNG/WebP, 800 px minimum, 10 Mo maximum et dix fichiers de galerie/nouveaux ; les URL d’aperçu sont libérées au retrait ou à la destruction. Les photos déjà enregistrées, leur couverture, leur ordre et leur retrait restent distincts. PHP ciblé : 39 tests / 392 assertions ; Playwright photo public desktop/mobile validé.

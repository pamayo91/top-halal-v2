...
- Added the separate `legacy_restaurant_authorships` provenance relationship and imported 7,567 exact, published legacy listing-author links. The import excluded 51 non-published source listings and 66 published listings without a V2 restaurant match; it did not create claims, change roles/permissions or recreate restaurants. The user list now shows this historical signal as `Fiches legacy créées` / `Auteur legacy`.

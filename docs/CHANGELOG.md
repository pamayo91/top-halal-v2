...
- Corrected restaurant outbound-link activation and rendering across public submissions, restaurateur edits and Filament; public destinations remain opaque `/sortie/{token}` routes only. Added PHP/Blade coverage for every supported network, invalid/inactive values, publication and Filament persistence, plus desktop/mobile Playwright validation of the real fixture and its 302 redirects.
- Restaurant outbound actions now submit a CSRF-protected opaque `POST /sortie` form and receive a 303 external redirect. The tokenized GET route and all `/sortie/{token}` HTML links are removed.
- Restaurant outbound forms retain the validated visual controls and open destinations in a new tab with native `_blank`/`noopener` attributes.
...
- Restaurants : une fiche restaurateur déjà dotée d’une catégorie ou d’un service sélectionné pouvait être bloquée par la validation native de la toute première case non cochée. La contrainte `required` est désormais synchronisée au niveau du groupe partagé : elle n’est active que si aucune option n’est cochée. Les règles métier serveur et les permissions restent inchangées.
- Restaurants : le sélecteur partagé de photos est désormais utilisé par la proposition publique et l’éditeur restaurateur. Les ajouts successifs se cumulent côté navigateur, les nouvelles images s’affichent immédiatement avec badge `Nouvelle`, nom et retrait avant sauvegarde, puis le `FileList` multipart est reconstruit sans upload anticipé. Les contrôles visibles reprennent exactement JPEG/PNG/WebP, 800 px minimum, 10 Mo maximum et dix fichiers de galerie/nouveaux ; les URL d’aperçu sont libérées au retrait ou à la destruction. Les photos déjà enregistrées, leur couverture, leur ordre et leur retrait restent distincts. PHP ciblé : 39 tests / 392 assertions ; Playwright photo public desktop/mobile et gate complet préproduction (80 contrôles) validés.
- Restaurants : l’éditeur « Modifier la fiche » adopte désormais la même UX que « Ajouter un restaurant » pour les cartes halal, les spécialités/services et les horaires. Les horaires utilisent un composant Blade et un module JS partagés avec états Fermé / 24 h/24 / Horaires, plages ajoutées ou retirées, recopie et validation des chevauchements ; le mapping canonique `day/status/slots` est validé par `RestaurantHours` dans les deux parcours. La page propriétaire occupe maintenant 1 120 px, utilise des grilles responsives, une galerie de miniatures avec dimensions de variante réelles et une barre de sauvegarde sticky, sans changement de policy, services métier, adresse, médias ou e-mail. Les tests ciblés passent (38 tests, 378 assertions) ; la suite PHP isolée complète n’a rencontré que la fixture legacy explicitement indisponible dans ce checkout.
...
- Validation préproduction : les parcours ciblés de l’éditeur partagé passent en desktop/mobile (trois profils, persistance réelle horaires/médias), la suite publique ciblée passe après conservation des sélecteurs de recopie et des contrôles visibles, et le gate de non-régression complet passe avec 80 contrôles navigateur/sentinelles et aucune nouvelle erreur Laravel.
...
- Stabilisé la grille de recopie publique afin que son bouton occupe toujours une ligne distincte des jours sélectionnables.
- Rendu obligatoires, côté interface et validation serveur, une catégorie/type de cuisine et un service/caractéristique pour chaque proposition publique de restaurant.
...
- Restaurants : une proposition déclarée gérant/propriétaire prépare désormais un unique claim `new_submission` différé. Il devient ownership officiel uniquement lors de la publication Top Halal ; une proposition non-gérante ne crée jamais de claim automatique.
...
- Documented the dedicated preproduction back-office test account contract: authenticated Playwright uses local-only `PREPROD_ADMIN_EMAIL` / `PREPROD_ADMIN_PASSWORD`, never a human administrator account or a repository secret.
...
- Emails : les clients Outlook utilisant le moteur Word reçoivent un bouton VML arrondi dédié ; les autres clients gardent le bouton HTML/table. Le conteneur utilise aussi un modèle de bordures séparées pour fiabiliser ses coins arrondis hors Outlook.
...
# 2026-09-13 — Workflow de signalement unifié
...
- Remplacé le focus bleu natif des champs publics par une bordure/ring safran accessible, et réchauffé le séparateur horizontal existant du footer sans modifier boutons, liens ou surface.

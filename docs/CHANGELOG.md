...
- Comptes : le rôle technique obsolète `restaurant_owner` est retiré et normalisé de façon idempotente vers `user`. Claims approuvés et soumissions restent les seules sources d’ownership ou de gestion ; aucun claim ni relation n’est créé par cette correction.

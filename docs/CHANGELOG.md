...
- Restaurant covers and galleries now accept only JPEG, PNG and WebP. Added a V2-only audited cleanup command that removes video relations, promotes the next eligible image, normalizes gallery order and can purge only truly orphaned V2 video files.

...
| App DB connection | BLOCKED | MariaDB config placeholders are in `.env.example`; real preprod DB credentials are not available yet |
| Legacy DB connection | BLOCKED | Legacy source is the current `meyo5199_th` MariaDB database using a dedicated SELECT-only user and WordPress prefix `tp_`; credentials not available yet |
...
| SEO validation | TODO | |
| Playwright E2E | SCAFFOLDED | `tests/e2e/preprod-smoke.spec.ts` added; cannot run against preprod until `.env` is configured and Apache exposes Laravel `public/` |

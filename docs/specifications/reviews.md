...
- The public form uses a native `fieldset` with five required 1–5 radio inputs, styled as stars without a JavaScript dependency. It labels the author field `Prénom ou pseudo`, keeps identity/e-mail fields in a two-column desktop grid (one column on compact screens), and reopens with the selected rating and field-local accessible errors after validation.

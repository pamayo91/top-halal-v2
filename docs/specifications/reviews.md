...
- A validated restaurant manager cannot submit a review for that restaurant. The single manager rule covers an approved claim (including `new_submission`), an owner-declared `restaurant_submission` once published, and an exact `legacy_restaurant_authorships` relation, but not a non-manager depositor. It is enforced again immediately before review creation, including after e-mail verification; the public form is hidden for a signed-in manager. A refused direct POST creates neither review, verification, operational notification nor delivery log.

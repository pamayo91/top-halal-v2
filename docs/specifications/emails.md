...
- `EmailTemplate` rows are administrative overrides over deterministic application defaults. A new transactional type is declared once in `EmailTemplateRegistry`; it then appears in Filament after opening `Emails > Templates`. Every declared transactional template is always deliverable. `EmailGlobalSettings` owns only the display-name fallback, managed-media logo and single plain-text footer; the header slogan is the existing `footer_navigation.introduction` setting. E-mail colours, button geometry and system font stack inherit the shared `design` configuration used by the public layout.
...
## Implemented events
- Email verification, password reset, password-change confirmation, claim received/accepted/refused, restaurant-submission e-mail verification, confirmation, refusal, operational-review alert and publication notifications use central templates. A proposal-refusal notification is queued exactly once after the atomic moderation decision, has no `Reply-To`, states that it is automatic and must not be answered, includes the optional administrator reason, and has a “Contester ce refus” CTA to the existing Contact form. Its opaque proposal reference is carried as minimal Contact context, not as personal data in the URL. The submission-confirmation template includes the seven-day, one-use contributor-space activation CTA only when the attached account needs activation; the verification, confirmation, operational-review and publication events remain queued for every valid submission. Restaurant-removal requests likewise queue a requester acknowledgement on creation, an acceptance or rejection result after Top Halal moderation, and (when configured) an operational alert to the same single recipient in `Contact > Réglages`. The single operational recipient in `Contact > Réglages` receives both Contact messages and these restaurant workflow alerts.
- Account e-mail replacement queues a confirmation to the proposed new address and, only after its atomic confirmation, a security alert to the former address. The delivery history stores neither token nor verification URL.
...
- Verified content reports use that same contributor-verification template when needed. Once a report is actually created, the configured operational Contact recipient receives the queued `content_report_admin_review` template with the context, reporter identity state and a BO CTA. There is no automatic acknowledgement or resolution/refusal e-mail to the reporter.
...
- The Laravel scheduler also runs `contributions:purge-verifications` daily. This separate retention action deletes only expired or consumed contribution-verification payloads older than seven days; it never deletes email-delivery history.

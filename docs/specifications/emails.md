...
- `EmailTemplate` rows are administrative overrides over deterministic application defaults. A new transactional type is declared once in `EmailTemplateRegistry`; it then appears in Filament after opening `Emails > Templates`. Every declared transactional template is always deliverable; `EmailGlobalSettings` owns the common 600px header/footer layout (display name, managed-media logo, primary colour, footer and optional dynamic year/additional text); template bodies remain plain text only.

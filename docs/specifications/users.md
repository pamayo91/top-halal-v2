...
- When that same verified e-mail later submits a restaurant, the exact same identity is attached to the submission. The restaurant flow enables login and requires a password choice without replacing the identity, so its existing reviews and comments remain linked to the same `user_id`.

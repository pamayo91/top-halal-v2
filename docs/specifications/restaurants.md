...
- Step 5 asks whether the contributor is the manager/owner. `Non` preserves the simple existing contribution flow. `Oui` requires an authenticated Top Halal account, name, company, a normalized and Luhn-valid 14-digit SIRET and a certification checkbox; no identity document is requested. An internal `pending_publication` ownership claim is activated only when the normal restaurant moderation workflow publishes the restaurant.

...
- Step 5 asks whether the contributor is the manager/owner using two accessible full-width radio cards. `Non` preserves the simple contribution flow and shows only the e-mail. `Oui` reveals name, company, a normalized and Luhn-valid 14-digit SIRET and a certification checkbox alongside the e-mail; no identity document or password is requested at this stage. For every role, successful e-mail confirmation creates or attaches a dedicated contributor account and sends a seven-day, one-use password-activation link in the confirmation e-mail. The account can manage only its submitted restaurant(s), including while moderation is pending. This access is not a `restaurant_claim`: the restaurant remains claimable by its actual manager.

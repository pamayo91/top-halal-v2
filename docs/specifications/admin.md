...
Les listes affichent les dates métier pertinentes en format français et dans le fuseau explicite `Europe/Paris` : publication et dernière modification legacy pour les contenus, date de commentaire/avis et de modération, inscription utilisateur, dates legacy des médias et restaurants. Les champs Filament date-heure appliquent le même fuseau à la saisie et à l’affichage, tandis que les instants techniques Laravel restent stockés en UTC. La liste des utilisateurs recherche à la fois le nom et toute portion de l’adresse e-mail. Elle expose aussi l’origine (`Migré WordPress` ou `Inscription V2`), le nombre de restaurants liés, les compteurs triables `Avis` et `Commentaires`, le détail des revendications et une activité (`Restaurateur`, `Revendication en cours`, `Revendication traitée`, `Auteur legacy` ou `Aucune activité`). Les contributions sont reliées à un compte par son identifiant WordPress historique ou par l’adresse e-mail de l’auteur, sans double comptage lorsqu’ils concordent. « Restaurants liés » additionne les revendications approuvées et les relations auteur WordPress historiques ; ces dernières restent une provenance, sans droit de gestion. Le filtre « Sans lien restaurant ni revendication » isole les comptes à examiner, sans les qualifier automatiquement de spam. La liste des restaurants expose « Publié le » (date legacy, sinon création V2) et ouvre par défaut les fiches les plus récemment ajoutées. Sa table masque progressivement les colonnes secondaires sur écran étroit ; le conteneur du panel reste fluide sans débordement de page. La géographie affiche le parent/niveau, le nombre de restaurants et la date de modification ; son filtre « Sans restaurant » rend les termes inutilisés immédiatement identifiables. Les valeurs manifestement techniques ou malveillantes sont refusées à la saisie.

...
Les secrets restent dans `.env`; les réglages V2 ne contiennent que des valeurs non sensibles. L’action « Supprimer » disponible dans la liste, dans les actions en masse et sur la fiche restaurant est un archivage réversible : elle retire la fiche des parcours publics, conserve les données et laisse une trace d’audit. Les taxonomies associées à des restaurants ne peuvent pas être supprimées.

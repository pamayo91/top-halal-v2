...
Les secrets restent dans `.env`; les réglages V2 ne contiennent que des valeurs non sensibles. L’action « Supprimer » disponible dans la liste, dans les actions en masse et sur la fiche restaurant place la fiche dans la « Corbeille » (soft delete) : elle est retirée des parcours publics, peut être restaurée et laisse une trace d’audit. Depuis la Corbeille, un administrateur peut supprimer définitivement une ou plusieurs fiches, ou vider toute la Corbeille après confirmation explicite ; ces suppressions sont irréversibles et auditées. Les taxonomies associées à des restaurants ne peuvent pas être supprimées.

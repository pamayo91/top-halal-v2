...
Chaque sentinelle restaurant déclare explicitement les invariants qui correspondent à son objet : `categories` ne contrôle que les catégories, les sentinelles média contrôlent les relations média/asset/variantes, `structured_address` contrôle seulement l'adresse/GPS et `features` seulement les services. Les contributions vivantes ne sont jamais snapshotées par une sentinelle sans rapport. La sentinelle `restaurant.reviews` protège en revanche chaque avis historique retenu (ID V2, rattachement au restaurant, identité legacy lorsqu'elle existe et statut) ; les avis V2 supplémentaires, `pending` ou approuvés, restent autorisés. Les volumes globaux ont un seuil minimal : une addition légitime est acceptée, une baisse exige une investigation et une mise à jour volontaire du baseline.

...
$phpTests = & ssh $PreprodHost "cd $PreprodPath && $remotePhp artisan config:clear && $remotePhp artisan route:clear && ($remotePhp artisan test --filter='RegressionSentinelTest|AdminBackOfficeTest|EditorialSlugStabilityTest' || { $remotePhp artisan config:cache; exit 1; }) && $remotePhp artisan config:cache"
if ($LASTEXITCODE -ne 0) {
    if ($phpTests) { Write-Error ($phpTests -join [Environment]::NewLine) }
    exit $LASTEXITCODE
}
